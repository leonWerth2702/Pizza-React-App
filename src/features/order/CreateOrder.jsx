...
function CreateOrder() {
    ...
    const {
        username, status: addressStatus, position, address, error: errorAddress
        ...
        <Form method={"POST"}>
            ...
            <div className={"mb-5 flex gap-2 flex-col sm:flex-row sm:items-center relative"}>
                ...
                {!position.latitude && !position.longitude && (
                    <span className={"absolute right-[3px] md:right-[5px] md:top-[5px] z-50"}>
            ...
            <div className={"mb-12 flex items-center gap-5"}>
                <input
                    className={"h-6 w-6 accent-yellow-400 focus:outline-none focus:ring focus:ring-yellow-400 focus:ring-offset-2 transition-all duration-100"}
                    type="checkbox"
                    name="priority"
                    id="priority"
                    value={`${withPriority}`}
